...
func TestSharedInformerWatchDisruption(t *testing.T) {
	...
	// Add pod3, but don't broadcast it
	// Bump pod2, but don't broadcast it
	...
	// Ensure that nobody saw pod2 update or pod3 add
	for _, listener := range listeners {
		if !listener.ok() {
			t.Errorf("%s: expected %v, got %v", listener.name, listener.expectedItemNames, listener.receivedItemNames)
		}
	...
	// Close and re-open the informer, simulating reconnect
	source.SwapWatch()
