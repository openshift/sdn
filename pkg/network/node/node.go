...
func (node *OsdnNode) validateMTU() error {
	...
	if mtu < int(node.mtu)+50 {
		return fmt.Errorf("interface MTU (%d) is too small for specified overlay MTU (%d)", mtu, node.mtu)
	}
	return nil
}
...
func (node *OsdnNode) reattachPods(existingPodSandboxes map[string]*kruntimeapi.PodSandbox, existingOFPodNetworks map[string]podNetworkInfo) error {
	pods, err := node.GetRunningPods(metav1.NamespaceAll)
	if err != nil {
		return err
	}
	start := time.Now()
