...
func (proxy *OsdnProxy) updateEgressNetworkPolicy(policy osdnv1.EgressNetworkPolicy) {
	...
	for _, pep := range ns.blockableEndpoints {
		...
		case wasBlocked && !pep.blocked:
			proxy.baseProxy.(EndpointsHandler).OnEndpointsAdd(pep.endpoints)
		case !wasBlocked && pep.blocked:
			proxy.baseProxy.(EndpointsHandler).OnEndpointsDelete(pep.endpoints)
		}
	}
...
func (proxy *OsdnProxy) OnEndpointsAdd(ep *corev1.Endpoints) {
	...
	proxy.baseProxy.(EndpointsHandler).OnEndpointsAdd(ep)
}
...
func (proxy *OsdnProxy) OnEndpointsUpdate(old, ep *corev1.Endpoints) {
	...
	if pep == nil {
		if !isBlockable {
			...
			proxy.baseProxy.(EndpointsHandler).OnEndpointsUpdate(old, ep)
			return
		}
	...
	case !wasBlocked && !isBlocked:
		proxy.baseProxy.(EndpointsHandler).OnEndpointsUpdate(old, ep)
	case !wasBlocked && isBlocked:
		proxy.baseProxy.(EndpointsHandler).OnEndpointsDelete(old)
	}
...
func (proxy *OsdnProxy) OnEndpointsDelete(ep *corev1.Endpoints) {
	...
	proxy.baseProxy.(EndpointsHandler).OnEndpointsDelete(ep)
}

func (proxy *OsdnProxy) OnEndpointsSynced() {
	proxy.baseProxy.(EndpointsHandler).OnEndpointsSynced()
